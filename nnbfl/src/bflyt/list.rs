...
impl BitPackable<u32> for MaterialInfo {
    fn decode(raw: u32) -> Self {
        Self {
            tex_map_count: (raw & 0b11) as u8,
            tex_srt_count: ((raw >> 2) & 0b11) as u8,
            tex_coord_gen_count: ((raw >> 4) & 0b11) as u8,
            tev_combiner_count: ((raw >> 6) & 0b111) as u8,
            has_alpha_compare: ((raw >> 9) & 0b1) != 0,
            has_color_blend_mode: ((raw >> 10) & 0b1) != 0,
            use_texture_only: ((raw >> 11) & 0b1) != 0,
            has_separate_blend_mode: ((raw >> 12) & 0b1) != 0,
            has_indirect_matrix: ((raw >> 14) & 0b1) != 0,
            projection_tex_gen_count: ((raw >> 15) & 0b11) as u8,
            has_font_shadow_parameter: ((raw >> 17) & 0b1) != 0,
            use_thresholding_alpha_interpolation: ((raw >> 18) & 0b1) != 0,
            use_detailed_combiner: ((raw >> 19) & 0b1) != 0,
            has_user_combiner: ((raw >> 20) & 0b1) != 0,
            has_texture_extensions: ((raw >> 21) & 0b1) as u8,
            vector_texture_info_count: ((raw >> 22) & 0b11) as u8,
            brick_repeat_shader_info_count: ((raw >> 24) & 0b11) as u8,
        }
    }
